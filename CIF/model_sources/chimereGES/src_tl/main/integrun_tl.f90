...
!  print*,' printout of the initial concentrations and meteo parameters'
  call outprint_tl(iprint,.true.,1,NetCDF_output,NetCDF_parout)
  call mpi_barrier(mpi_comm_world,ierr)
...
!  print*,' Loop on physical time steps'
  do nh=1,nhourrun
    ...
    do np=1,nphour
      ...
      call mpi_barrier(mpi_comm_world,ierr)
      call master_locvalues_tl ! also sends conc boundaries
      call mpi_barrier(mpi_comm_world,ierr)
      ...
      if(np.eq.nphour) then
        call master_recv_locvalues
        iprint = iprint+1
        call outprint_tl(iprint,.false.,nh,NetCDF_output,NetCDF_parout) ! also receives conc from workers
      endif
   enddo ! np=1,nphour
