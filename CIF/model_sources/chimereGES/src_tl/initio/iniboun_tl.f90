...
! Boundary concentration increment file is already open
      ...
      !   Getting dimensions IDs
      !     Are Boundary concentrations time-dependant ?
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'Time',timedimid)
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'DateStrLen',datedimid)
      NCERR(__LINE__)
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'h_boundary',  hbdimid)
      NCERR(__LINE__)
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'bottom_top',  btdimid)
      NCERR(__LINE__)
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'Species',     spdimid)
      NCERR(__LINE__)
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'west_east',   wedimid)
      NCERR(__LINE__)
      ncstat=nf90_inq_dimid(bounconcincr_ncid,'south_north', sndimid)
      NCERR(__LINE__)

      !   Checking lengths
      ncstat=nf90_inquire_dimension(bounconcincr_ncid,timedimid,len=ntimes)
      NCERR(__LINE__)
      ncstat=nf90_inquire_dimension(bounconcincr_ncid,datedimid,len=dl)
      NCERR(__LINE__)
      ncstat=nf90_inquire_dimension(bounconcincr_ncid,hbdimid,len=hb)
      NCERR(__LINE__)
      ncstat=nf90_inquire_dimension(bounconcincr_ncid,btdimid,len=bt)
      NCERR(__LINE__)
      ncstat=nf90_inquire_dimension(bounconcincr_ncid,wedimid,len=we)
      NCERR(__LINE__)
      ncstat=nf90_inquire_dimension(bounconcincr_ncid,sndimid,len=sn)
      NCERR(__LINE__)
      if (ntimes<nhourrun) then
        print *, 'iniboun : Boundary concentration increment file '
        print *, 'does not contain enough time slots. Exiting.'
        call exit1('Exiting')
      end if
      if (dl /= dlen)        call exit1('iniboun : date format error in Boundary concentration increment file')
      if (hb /= nhbound_domain)     call exit1(&
          'iniboun : h_boundary dimension inconsistency in Boundary concentration increment file')
      if (bt /= nverti)      call exit1(&
          'iniboun : bottom_top dimension inconsistency in Boundary concentration increment file')
      if (we /= nzonal_domain)      call exit1(&
          'iniboun : west_east dimension inconsistency in Boundary concentration increment file')
      if (sn /= nmerid_domain)      call exit1(&
          'iniboun : west_east dimension inconsistency in Boundary concentration increment file')

      ! Checking time
      ncstat=nf90_inq_varid(bounconcincr_ncid,'Times', bounconcincr_times_varid)
      NCERR(__LINE__)
      ncstat=nf90_get_var( &
    ...
    do ns=1,nspec
	   ...
	   boundlat_tl(nl,ns,1) = 0d0
           boundlat_tl(nl,ns,2) = 0d0
...
NCERR(__LINE__)
    ...
    deallocate(buf3)
      ...
      ! Reading lateral concentration increments in ppb
      ncstat=nf90_inq_varid(bounconcincr_ncid,'lat_conc', latconcincr_conc_varid)
      NCERR(__LINE__)
      stvec4 = (/1        , 1      , 1,      1/)
      cntvec4= (/nspecboun, nhbound_domain, nverti, 1/)
      allocate(buf3(nspecboun, nhbound_domain, nverti))
      ...
      do ivert=1,nverti
        do ih=1,nhbound_domain
           ...
           do ns=1,nspecboun
              if(isboun(ns).gt.0) then
                 boundlat_tl(nl,isboun(ns),1) = buf3(ns,ih,ivert)
              end if
           end do
        end do
      end do
      ...
      ncstat=nf90_get_var( &
          bounconcincr_ncid, bounconcincr_times_varid, datebuf,(/1,2/),(/dlen,1/))
      NCERR(__LINE__)
      idex = idate(1)
      idr=mm5date2numeric(datebuf) 
      if(idr.ne.idex) then
        print *,'*** ERROR: WRONG EXPECTED DATE IN BOUN_CONCS INCR FILE'
        print *,'IHOURRUN=1,  EXPECTED= ',idex,' BOUN_CONCS= ',idr
        call exit1('Exiting')
      endif
      stvec4 =(/1        , 1      , 1,      2/)
      ...
      do ivert=1,nverti
        do ih=1,nhbound_domain
           ...
           do ns=1,nspecboun
              if(isboun(ns).gt.0) then
                 boundlat_tl(nl,isboun(ns),2) = buf3(ns,ih,ivert)
              end if
           end do
        end do
      end do
    ...
    do ns=1,nspec
        do ime=1,nmerid_domain
           do izo=1,nzonal_domain
	            boundtop_tl(izo,ime,ns,1) = 0d0
              boundtop_tl(izo,ime,ns,2) = 0d0
...
! Reading top concentration increments in ppb
      ncstat=nf90_inq_varid(bounconcincr_ncid,'top_conc', topconcincr_conc_varid)
      NCERR(__LINE__)
      stvec4 =(/1       , 1     , 1     , 1/)
      cntvec4=(/nspecboun, nzonal_domain, nmerid_domain, 1/)
      allocate(buf3(nspecboun, nzonal_domain, nmerid_domain))
      ...
      do ime=1,nmerid_domain
        do izo=1,nzonal_domain
           do ns=1,nspecboun
              if(isboun(ns).gt.0) then
                 boundtop_tl(izo,ime,isboun(ns),1) = buf3(ns,izo,ime)
              end if
           end do
        end do
      end do
      ...
      do ime=1,nmerid_domain
        do izo=1,nzonal_domain
           do ns=1,nspecboun
              if(isboun(ns).gt.0) then
                 boundtop_tl(izo,ime,isboun(ns),2) = buf3(ns,izo,ime)
              end if
           end do
        end do
      end do
