...
NCERR(__LINE__)
      ...
      if(z_units(1:3).eq."ppb") then
        concini(ns,:,:,:)=buf3d*airm(1:nzonal_domain,1:nmerid_domain,1:nverti,1)*1d-9
        ...
        if(species(ns)%bounddry.eq.1) then
          fdry = m_h2o * (1 - sphu(1:nzonal_domain,1:nmerid_domain,1:nverti,1) / airm(1:nzonal_domain,1:nmerid_domain,1:nverti,1) / 1.6) &
          ...
          conc(1:nzonal_domain,1:nmerid_domain,1:nverti) = conc(1:nzonal_domain,1:nmerid_domain,1:nverti) * fdry
        end if
      else
        concini(ns,:,:,:)=buf3d
        conc(1:nzonal_domain,1:nmerid_domain,1:nverti)=buf3d
        ...
        if(z_units(1:3).eq."ppb") then
          ...
          if(species(ns)%bounddry.eq.1) then
            fdry(1:nzonal_domain, 1:nmerid_domain, 1:nverti) = &
                ...
                / (m_air * sphu(1:nzonal_domain,1:nmerid_domain,1:nverti,1) / airm(1:nzonal_domain,1:nmerid_domain,1:nverti,1) / 1.6 &
                    + m_h2o * (1 - sphu(1:nzonal_domain,1:nmerid_domain,1:nverti,1) / airm(1:nzonal_domain,1:nmerid_domain,1:nverti,1) / 1.6))
            conco(1:nzonal_domain,1:nmerid_domain,1:nverti) = conco(1:nzonal_domain,1:nmerid_domain,1:nverti) * fdry
          end if
        else
          conco(1:nzonal_domain,1:nmerid_domain,1:nverti)=buf3d
        endif
...
contains
  ...
  end subroutine duplicate_boundary
