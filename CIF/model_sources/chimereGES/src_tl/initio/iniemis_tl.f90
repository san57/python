...
5422    continue
 ...
 ! Anthropic emission  increment file (netCDF) is already open
    ! Checking dimensions consistency between file and chimere parameters
  ! dimensions IDs
  ncstat=nf90_inq_dimid(emisaincr_ncid,'Time',atimedimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisaincr_ncid,'west_east',awedimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisaincr_ncid,'south_north',asndimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisaincr_ncid,'bottom_top',abtdimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisaincr_ncid,'SpStrLen',aspstrdimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisaincr_ncid,'DateStrLen',adatestrdimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisaincr_ncid,'Species',nadimid)
  NCERR(__LINE__)
  ! lengths
  ncstat=nf90_inquire_dimension(emisaincr_ncid,atimedimid,len=ntimes)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisaincr_ncid,awedimid,len=we)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisaincr_ncid,asndimid,len=sn)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisaincr_ncid,aspstrdimid,len=spl)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisaincr_ncid,adatestrdimid,len=dl)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisaincr_ncid,nadimid,len=na)
  NCERR(__LINE__)
  if (we/=nzonal_domain)      call exit1('iniemis : west_east dimension inconsistency')
  if (sn/=nmerid_domain)      call exit1('iniemis : south_north dimension inconsistency')
  if (dl/=dlen)        call exit1('iniemis : date format error in A-emissions INCR file')
  if (spl/=splen)      call exit1('iniemis : species name format error in A-emissions INCR file')
  ! checking time
  ncstat=nf90_inq_varid(emisaincr_ncid,'Times', emisaincr_times_varid)
  NCERR(__LINE__)
  !! first hour
  ncstat=nf90_get_var( &
       emisaincr_ncid, &
  ...
  ide=mm5date2numeric(datebuf)
  if(ide.ne.idex) then
    print *,'*** ERROR: WRONG EXPECTED DATE IN AEMISSIONS INCREMENT FILE'
    print *,'IHOURRUN=0,  EXPECTED= ',idex,' AEMISSIONSINCR= ',ide
    call exit1('Exiting')
  endif
  !! next hour
  ncstat=nf90_get_var( &
  ...
  if(ide.ne.idex) then
    print *,'*** ERROR: WRONG EXPECTED DATE IN AEMISSIONS INCREMENT FILE'
    print *,'IHOURRUN=1,  EXPECTED= ',idex,' AEMISSIONSINCR= ',ide
    call exit1('Exiting')
  endif
...
NCERR(__LINE__)
    ncstat=nf90_inq_varid(emisaincr_ncid,charspec,emisaincrvarid(ne))
NCERR(__LINE__)
  end do
  close(ifnanthro)
...
!  ncstat=nf90_inq_dimid(emisb_ncid,'biospecies',nbiodimid)
 ...
 ! Biogenic emission increment file (netCDF) is already open
  ! Checking dimensions consistency between file and chimere parameters
  ! dimensions IDs
  ncstat=nf90_inq_dimid(emisbincr_ncid,'Time',btimedimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'DateStrLen',bdatestrdimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'west_east',bwedimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'south_north',bsndimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'bottom_top',bbtdimid)
  NCERR(__LINE__)
!  ncstat=nf90_inq_dimid(emisb_ncid,'biospecies',nbiodimid)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'SpStrLen',bspstrdimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'DateStrLen',bdatestrdimid)
  NCERR(__LINE__)
  ncstat=nf90_inq_dimid(emisbincr_ncid,'Species',nbiodimid)
  NCERR(__LINE__)
  ! lengths
  ncstat=nf90_inquire_dimension(emisbincr_ncid,btimedimid,len=ntimes)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisbincr_ncid,bdatestrdimid,len=dl)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisbincr_ncid,bwedimid,len=we)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisbincr_ncid,bsndimid,len=sn)
  NCERR(__LINE__) 
   ncstat=nf90_inquire_dimension(emisbincr_ncid,bbtdimid,len=blev)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisbincr_ncid,nbiodimid,len=mbio)
  NCERR(__LINE__)
  ncstat=nf90_inquire_dimension(emisbincr_ncid,bspstrdimid,len=spl)
  NCERR(__LINE__)

  if (dl/=dlen)        call exit1('iniemis : date format error in input file')
  if (we/=nzonal_domain)      call exit1('iniemis : west_east dimension inconsistency')
  if (sn/=nmerid_domain)      call exit1('iniemis : south_north dimension inconsistency')
  if(blev/=1)          call exit1('iniemis : number of levels for B-emissions INCR not equal to 1')
  if (spl/=splen)      call exit1( 'iniemis : species name format error in B-emissions INCR file')
!  checking time
  ncstat=nf90_inq_varid(emisbincr_ncid,'Times', emisbincr_times_varid)
  NCERR(__LINE__)
  !! first hour
  ncstat=nf90_get_var( &
       emisbincr_ncid, &
  ...
  ide=mm5date2numeric(datebuf)
  if(ide.ne.idex) then
     print *,'*** ERROR: WRONG EXPECTED DATE IN BEMISSIONS INCREMENT FILE'
     print *,'IHOURRUN=0,  EXPECTED= ',idex,' BEMISSIONSINCR= ',ide
     call exit1('Exiting')
  endif
  !! next hour
  ncstat=nf90_get_var( &
  ...
  if(ide.ne.idex) then
     print *,'*** ERROR: WRONG EXPECTED DATE IN BEMISSIONS INCREMENT FILE'
     print *,'IHOURRUN=1,  EXPECTED= ',idex,' BEMISSIONSINCR= ',ide
     call exit1('Exiting')
  endif
  ...
  do ne=1,nemisb
     ...
     NCERR(__LINE__)
     ncstat=nf90_inq_varid(emisbincr_ncid,charspec,emisbincrvarid(ne))
     NCERR(__LINE__)
