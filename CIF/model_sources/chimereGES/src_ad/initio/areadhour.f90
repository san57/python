...
#define NCERR(lnum) if(ncstat/=NF90_NOERR) call nc_err(ncstat,lnum,'areadhour.f90')
  ...
  real(kind=8), allocatable,dimension(:,:)   :: buf2
        ...
        do ilev=1,nlevemis
           do ime=1,nmerid_domain
              do izo=1,nzonal_domain
                 emisaloc(ne,izo,ime,ilev)=buf4(izo,ime,ilev)
              enddo
           enddo
        enddo
     enddo
...
NCERR(__LINE__)
    ...
    if(ide.ne.idex) then
       ...
       print *,'IHOURRUN+1=',ihourrun,' EXPECTED: ' &
          ,idex,' BEMISSIONS: ',ide
       call exit1('Exiting')
    endif
    do ne=1,nemisb
      ncstat=nf90_get_var( &
       ...
       emisbvarid(ne), &
       emisb(ne,:,:,ksens), &
       (/1,1,1,ihourrun+1/),(/nzonal_domain,nmerid_domain,1,1/))
NCERR(__LINE__)
  enddo
